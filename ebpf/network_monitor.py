...
Attaches kprobes/kretprobes to tcp_v4_connect and tcp_v6_connect.
Emits a schema-compliant net_connect event for every successful
outbound TCP connection (IPv4 and IPv6).

Strategy: save the sock pointer at connect() entry (keyed by TID),
then read addresses on the return probe after the kernel has filled
them in and the connection has either succeeded or failed.

Requires: root, kernel eBPF support, BCC installed.
"""
import os
import signal
import socket
...
# Loopback connections are high-volume noise for most EDR detection use cases.
# Set False to capture localhost traffic (useful for debugging sensors locally).
...
struct ipv4_event_t {
    ...
    u32  saddr;   /* local addr  — network byte order */
    u32  daddr;   /* remote addr — network byte order */
    u16  sport;   /* local port  — network byte order */
    u16  dport;   /* remote port — network byte order */
};
...
/*
 * BPF_HASH keyed by TID (not PID) so threads in the same process don't
 * collide when they call connect() concurrently.
 */
BPF_HASH(currsock, u32, struct sock *);
BPF_PERF_OUTPUT(ipv4_events);
BPF_PERF_OUTPUT(ipv6_events);

/* ── Entry probes — save sock pointer ────────────────────────────────────── */
...
/* ── Return probes — emit event only on success ───────────────────────────── */
...
int trace_connect_v4_return(struct pt_regs *ctx) {
    ...
    if (ret != 0) return 0;  /* connect() failed — skip */

    struct sock *sk = *skpp;

    /* Verify address family before reading IPv4-specific fields. */
    u16 family = 0;
...
def main():
    ...
    def handle_ipv4(cpu, data, size):
        ...
        ppid, _ = get_ppid_uid(pid)   # uid already captured in-kernel; skip /proc for it
